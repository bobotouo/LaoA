...
function App() {
  ...
  const { meta, indices, breadth, turnover, limitPool, hotLeaders = [], aPlusPicks = {}, marketRisk = {} } = data;
  ...
  // 大盘风险等级映射到颜色/文案
  const riskLevelMeta = {
    LOW: { label: "正常", cls: "risk-low", advice: "可按既定策略执行" },
    ELEVATED: { label: "警惕", cls: "risk-elevated", advice: "降仓收紧止损" },
    HIGH: { label: "高风险", cls: "risk-high", advice: "半仓以下" },
    SEVERE: { label: "极端风险", cls: "risk-severe", advice: "清仓观望" },
  };
  const marketRiskAssess = (marketRisk.assessments || [])[0];
  const prevAssess = (marketRisk.assessments || [])[1];
  const riskMeta = riskLevelMeta[marketRiskAssess?.level] || riskLevelMeta.LOW;
